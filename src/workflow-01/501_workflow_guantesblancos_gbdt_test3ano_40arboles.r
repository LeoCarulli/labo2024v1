...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp22/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow22/"
envg$EXPENV$wf_dir_local <- "~/flow22/"
...
FE_historia_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$RandomForest$num.trees <- 40
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    min_sum_hessian_in_leaf = 0.28, #  min_sum_hessian_in_leaf >= 0.0
    ...
    pos_bagging_fraction = 0.50, # 0.0 < pos_bagging_fraction <= 1.0
    neg_bagging_fraction = 0.50, # 0.0 < neg_bagging_fraction <= 1.0
    ...
    num_leaves = c( 50L, 1000L, "integer" )
  )
