...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp45/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow45/"
envg$EXPENV$wf_dir_local <- "~/flow45/"
...
TS_strategy_guantesblancos_202107 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$final_train <- c(
      202105, 202104, # Acá pongo los dos considerados en el validation y testing del training
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    lambda_l1 = c( 0L, 1000L, "integer" ), # lambda_l1 >= 0.0
    lambda_l2 = c( 0L, 1000L, "integer" ), # lambda_l2 >= 0.0

    min_gain_to_split = c( 0.1, 20.0 ), # min_gain_to_split >= 0.0
    feature_fraction = c( 0.1, 0.7 ),
    min_data_in_leaf = c( 10L, 5000L, "integer" ),   
